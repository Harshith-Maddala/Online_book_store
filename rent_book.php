...
$sql = "SELECT * FROM `book_info` WHERE book_id not in (select book_id from books_assigned)";
$result = mysqli_query($conn, $sql);
// Find the number of records returned
$num = mysqli_num_rows($result);
if($num> 0){
...
echo "<table style='width:100%'>
 ...
 <tr>
   ...
   <td>".$row['year']."</td>
   <td>".$row['genre']."</td>
   <td><input type='Button' value='Rent' onclick=\"rentabook('".$regno."','".$row['book_id']."')\"></td>
 </tr>"; 
 } 
 echo "</table>";
 echo "<script>
        function rentabook(regno, bid){
            window.location.href='rent_book_process.php?regno=$regno&bookid=$book_id';
        }
    </script>";
}
else{
    echo "No bookes found for rent";
   }
}
}
?>
